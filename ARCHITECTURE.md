...
| GET | `/report?from=&to=` | (v2.80.0) Date-range-driven dashboard data for page 1: `{ trend: {label,count,cost}[], byType: {type,label,count}[], total, totalEstimatedCost }`. Bucketing mirrors `accountingService.getSalesReport` (daily ≤92 days, else monthly), replicated locally rather than cross-imported |
