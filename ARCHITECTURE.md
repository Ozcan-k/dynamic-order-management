...
- **Employees** — add form (Department dropdown → First Name → Last Name → Start Date → Add), then a department-grouped list with columns **Employee ID (#101…) · Department · Name · Start Date · Edit/Delete**. `empNo` auto-assigned via an atomic per-tenant counter (starts at #101).
