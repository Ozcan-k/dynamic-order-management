...
| created_at | TIMESTAMPTZ | |
| employee_id | UUID FK NULL | → emp_employees, `ON DELETE SET NULL` (v2.88.0). The Employee Schedule record this login belongs to; one employee can own several logins (e.g. picker + packer). Set via Settings → Employee ID or Employee Schedule → *Linked system logins*. |
...
| carrier_name | VARCHAR | Logistics carrier (e.g. `SPX`, `SHOPEE_INSTANT` (v2.86.1 — "Shopee Instant"), `JT_EXPRESS`, `FLASH`, `LEX`, `LBC`, `NINJA_VAN`, `OTHER`). **Required** at Bulk Scan time. |
...
| Table | Purpose | Key fields |
|---|---|---|
| `incidents` | One row per filed incident | `tenantId`, `incidentType` (enum), `incidentDate`, `employeeUserId/FullName/Email`, `recipientEmail`, `reportedByUserId/FullName/Role`, `adminDescription` (text), `trackingNumber?`, `platform?`, `shopName?`, `costAmount?` (Decimal 12,2, ₱), `costQuantity?` (Int), `shippingCost?` (Decimal 12,2, ₱, v2.81.0), `signedFilePath?`, `signedFileMime?`, `signedUploadedAt?`, `emailSentAt?`, `emailSentTo?`, `createdById`, `createdAt`, `updatedAt` |
...
As of v2.80.0, a second, overlapping list of 6 loss-related types (`WRONG_ITEM_PICKED`, `WRONG_ITEM_PACKED`, `MISSING_ITEM`, `WRONG_QUANTITY`, `PARCEL_DAMAGE`, `LOST_PARCEL` — the first six enum values) require a mandatory estimated cost + quantity, centralised the same way (`COST_INCIDENT_TYPES` + `requiresCostContext()` in `shared/src/index.ts`). The two lists are distinct (4 vs 6 members) and independently checked — a type can require parcel context, cost context, both, or neither.
...
| GET | `/stats` | `{ total, thisMonth, topType: {type,count} \| null, smtpConfigured }` (all-time, independent of any date range) |
| GET | `/report?from=&to=` | (v2.80.0; extended v2.81.0) Date-range-driven dashboard data for page 1: `{ trend: {label,count,cost}[], byType: {type,label,count}[], byEmployeeCost: {employeeUserId,employeeFullName,cost}[], total, totalCost }`. `totalCost`/bucket `cost` = `costAmount + shippingCost` summed; `byEmployeeCost` only lists employees with cost > 0 in range (no full-roster join). Bucketing mirrors `accountingService.getSalesReport` (daily ≤92 days, else monthly), replicated locally rather than cross-imported |
| GET | `/pivot?from=&to=` | Employee × type pivot matrix sorted by total desc, each row now also carrying `totalCost` (₱). `from`/`to` are optional (v2.80.0) — omitted means all-time, unchanged for existing callers |
| GET | `/types` | The 25 IncidentType values with their human labels + `requiresParcel` + `requiresCost` (v2.80.0) flags |
...
- `frontend/src/pages/IncidentReport.tsx` — **page 1 (dashboard), redesigned v2.80.0; extended v2.81.0.** Page hero (logo + company name + 2 CTAs) · a single `DateRangePicker` (reused as-is from `components/accounting/DateRangePicker.tsx`; All/This Month/Last Month/This Year/Custom) driving everything below it · 5 stat cards (Total Incidents · This Month · Top Incident Type · **Total Cost** [renamed from "Estimated Cost" in v2.81.0, now Estimated+Shipping] · Email Delivery) · recharts **trend chart** (incidents per day/month for the selected range) · recharts **breakdown-by-type chart** (horizontal bars, click a bar to filter the table below) · recharts **"Cost by Employee" chart** (v2.81.0, same horizontal-bar style, only employees with cost > 0 in range appear) · filter card (search + type) · Recent Incidents table (25/page, Edit/Open/Delete actions) · a "View Incident Count by Employee →" CTA card that navigates to page 2 carrying the current date range as query params. The old employee×type pivot table has moved to page 2.
- `frontend/src/pages/IncidentEmployeeReport.tsx` — **page 2 (employee drill-down), new in v2.80.0.** Reads `from`/`to` via `useSearchParams()`, "← Back to Report" button. Primary view is a ranked leaderboard (rank + initials avatar + relative bar + total incidents + estimated cost + that employee's top incident type), modeled on Accounting Report's `AgentLeaderboard`. The original dense employee×type matrix is retained underneath a "Show full type breakdown" toggle.
- `frontend/src/pages/incident/CreateIncidentModal.tsx` — wide modal with grid form, reused for both create and **edit** (optional `editing` prop). Conditional 3-field parcel block (TN + Platform + Shop) appears only for the 4 parcel-context types. Conditional **Estimated Loss block** (v2.80.0, extended v2.81.0: Estimated Cost ₱ + Quantity Affected + **Shipping Cost ₱**, all three mandatory) appears only for the 6 cost-context types, derived from `requiresCost` on the selected type (same pattern as the parcel block's `requiresParcel`). TN "Lookup" button calls `/incidents/lookup-tn` to auto-fill Platform + Shop from a matching Order. Employee dropdown auto-fills Full Name (from `getRememberedFullName()`) + Email (from `User.email`). Reported By block auto-fills from the current admin session (create mode only)
- `frontend/src/pages/incident/ViewIncidentModal.tsx` — opened from any row in Table A. Shows admin description + **Estimated Cost / Shipping Cost / Total Cost rows** (v2.80.0, extended v2.81.0; ₱-formatted via `money()` from the Accounting module, shown only when the incident has cost data) + 3-button action row (Download PDF · Upload Signed · Send Email) + status of signed/email. PDF + signed file are fetched as authenticated blobs via the api client (`downloadIncidentPdf`/`downloadSignedFile`) — a plain `<a href>` would hit the SPA fallback and render the login screen
...
**Invoice attachments — Sales + Expenses (v2.77.0, extended to Sales in v2.82.0):** a sale or expense can carry the document(s) that back it — photo or PDF, **more than one per record**. Two tables, one shape: `AccExpenseAttachment` (`expenseId` cascade) and `AccSaleAttachment` (`saleId` cascade), both `category`/`fileName`/`mime`/`originalName`/`sizeBytes`/`uploadedAt`. The service layer is kind-scoped (`AttachmentKind = 'sale' | 'expense'`) like categories/items — one set of functions, the Prisma call branches per kind since the two tables FK to different parents. The bytes live on the `backend_uploads` volume under `accounting/`, and the row stores **only the file name** (never an absolute path, so a change of `UPLOADS_ROOT` — dev Windows vs prod `/app/uploads` — cannot orphan it; this is the latent bug `IncidentDocument.filePath` still carries). Endpoints, mirrored under both `/accounting/sales/:id/attachments*` and `/accounting/expenses/:id/attachments*`: `GET|POST .../attachments` (`?category=INVOICE|PAYMENT_PROOF`, default `INVOICE`), `GET|DELETE .../attachments/:attId`, and **`PUT .../attachments/:attId`** (replace — see below), all under the existing `ADMIN + ACCOUNTANT` guard. Uploads are capped at **10 MB**, MIME-whitelisted to PDF/PNG/JPG/WEBP (**SVG is deliberately excluded** — it is script-bearing and these files are served back `inline`), served with `X-Content-Type-Options: nosniff`, and a re-upload of the same name+type **within the same category** returns **409** (an invoice and a payment proof may share a file name without conflicting). Oversize returns a friendly **413** (`@fastify/multipart` throws from `toBuffer()`; the `truncated` check is a fallback — see SOLUTIONS `[2026-06-21]`).

*Categories — Invoice vs Payment Proof (v2.83.0).* `AccAttachmentCategory` enum (`INVOICE` | `PAYMENT_PROOF`), column added to both attachment tables as `@default(INVOICE)` — additive, and every pre-existing row is correctly reclassified as an invoice for free. The two categories are entirely separate lists (separate query, separate upload button, separate section on the page) sharing one implementation — same shape as the Sales/Expense catalog `kind` split.

*Replace (v2.82.0).* Each attachment row has a **Replace** button alongside Download/Remove — picking a new file swaps the content behind that *same* attachment id (`replaceAttachment`): the new bytes are written under a fresh file name, the DB row is updated in place (`fileName`/`mime`/`originalName`/`sizeBytes`/`uploadedAt`), and only then is the old file unlinked. This keeps the row's position, id, and category stable — a plain Remove + re-Add would create a new row at the end of the list instead.

*Storage discipline.* Photos are **downscaled in the browser before upload** (`lib/imageDownscale.ts`: long edge → 1600 px, JPEG q0.82, skipped under 600 KB or if the result is larger) — applied to invoices, payment proofs, and Replace uploads alike. A ~3 MB phone photo lands around 400 KB. This matters because these are statutory records (BIR, ~10 years) and are therefore **exempt from every retention/purge job** — the volume only ever grows, so the ~8× saving is what keeps it bounded. PDFs pass through untouched.

*File lifecycle.* Soft-deleting a sale/expense **keeps** its files (the Recycle Bin must be able to restore it). Only a permanent purge unlinks them, and `purgeExpense`/`purgeSale` read the file names **before** the delete — the cascade would otherwise drop the attachment rows and with them the only record of which files on disk belonged to that record, leaving orphans nobody can find. Deleting a single attachment unlinks immediately.

*UI.* Two instances of a shared section component (`components/accounting/InvoiceAttachments.tsx`, `resource: 'sales' | 'expenses'`, `category: 'INVOICE' | 'PAYMENT_PROOF'`) sit under Items/Note in `InvoiceForm` and `PurchaseForm` — **Attachments / + Add Invoice File** first, **Payment Proof / + Add Payment Proof** directly below it as its own section with its own staged-file queue. **Hovering** a saved or queued file's thumbnail opens a large popup — the actual image enlarged for JPG/PNG/WEBP, a PDF icon + filename card for PDFs (no client-side PDF rendering). The popup is rendered via `createPortal` straight to `document.body` at `position: fixed`, positioned off the thumbnail's live bounding rect (flips to the opposite side / clamps within the viewport near an edge) — this is deliberate: an in-flow `position: absolute` popup was tried first and got clipped/hidden behind sibling rows depending on the parent's overflow, which a body-level portal is immune to. In edit mode a file uploads on selection. On a **new** record there is no id yet, so files queue client-side per category and flush (in parallel) after the save returns one; if a flush partially fails the form stays put, keeps the failed files, and records the new id so pressing Save again is a `PUT` retry rather than a duplicate `POST`. Previews and downloads fetch **blobs through the api client** — a bare `<img src>` or `window.open` sends no credentials and gets nginx's SPA fallback, i.e. the login screen (SOLUTIONS `[2026-05-02]` / `[2026-06-03]`).
...
### 7.14 Employee Schedule Module ✅ Built (v2.66.0; v2.84.0 adds the optional `userId` link to picker/packer logins used by Warehouse Report → Performance) — independent staff attendance scheduler · local `db push` PENDING (dev pg down → applies on CD deploy)

A self-contained workforce-scheduling module placed **directly under Incident Report** in the sidebar (single entry "Employee Schedule"). **Never touches the order pipeline, existing tables, or existing reports** — own `emp_*` tables, tenant-scoped, no FKs into existing models (same independent-module pattern as Accounting/Incident/Dispatch). Access: **ADMIN + WAREHOUSE_ADMIN** (view + edit) and **INCIDENT_REPORTER (view-only, v2.68.0)** — GET routes allow the reporter; all mutations stay Admin/Warehouse-Admin (403 otherwise); the UI hides every edit control + shows a "View Only" badge for the read-only role. All UI in English. Full plan: `EMPLOYEE_SCHEDULE.md`. shared + backend `tsc` and frontend `tsc -b && vite build` green.
...
**Attendance (fixed enum `AttendanceStatus`):** Present (8h) · Half Day (4h) · Absent · **Day Off** (v2.69.0) · Vacation Leave · Sick Leave · Maternity Leave (all non-Present/Half = 0h); blank "—" = unscheduled (not counted). **OT** dropdown (0–5h, default 0) appears only when Present. Day hours = base(status) + OT; the left "clock" shows the employee's weekly total (`HH:MM`).
...
- **Employees** — add form (Department → First Name → Last Name → Start Date, plus optional **Contact Number · Email · Address · Birthday · Emergency Contact Name · Emergency Contact Number**), then a department-grouped list of **active** employees (**Employee ID 1001… · Name (+ linked `@login` chips) · Contact · Start Date · Edit/Set Inactive/Delete**). `empNo` auto-assigned via an atomic per-tenant counter; since v2.86.0 IDs are **4 digits with no `#`** (old 3-digit numbers shifted +900 at startup by `migrateEmpNosToFourDigit`, e.g. #104 → 1004). The Edit modal's **Linked system logins** field (v2.88.0) links one or more system logins of any role to the employee. New employees default **Active**. **Active/inactive lifecycle (v2.67.0):** *Set Inactive* opens a modal that requires a **Leave Date**, then the employee drops to a separate **"Inactive / Former Employees"** table (ID · Name · Department · Start Date · **Leave Date** · Edit/Reactivate/Delete); *Reactivate* clears the leave date. The Edit modal exposes every field + a Status select (Inactive → required Leave Date). Inactive staff are excluded from the Schedule grid but remain in the Report for periods they worked. Delete is still a hard delete (cascade).
- **Report** — Weekly | Monthly toggle + period navigator; 4 summary cards (Total Employees / Worked Days / Hours / OT); a department-grouped per-employee table (Present / Half Day / Absent / Day Off / Vacation / Sick / Maternity / OT / **Worked Days** = present + 0.5·halfDay / **Total Hours** = 8·present + 4·halfDay + OT) with department subtotals + grand total, plus **CSV + PDF export**. Decisions (confirmed): blank "—" default (unscheduled, not counted), hard delete (cascade), week starts Sunday.

**Data model (`emp_*`, tenant-scoped, no existing-table FKs):** `EmpEmployee` (`empNo` Int, `@@unique([tenantId, empNo])`, department/firstName/lastName/`startDate @db.Date`; v2.67.0 +optional `contactNumber`/`email`/`address`/`birthday @db.Date`/`emergencyContactName`/`emergencyContactNumber` + `isActive Boolean @default(true)` + `leaveDate @db.Date?`), `EmpSchedule` (`date @db.Date`, `status`, `otHours`, `@@unique([tenantId, employeeId, date])`, `employee onDelete: Cascade`), `EmpCounter` (per-tenant `empNo` sequence, default 1000 since v2.86.0 → first ID 1001). Login link (v2.88.0): `User.employeeId` → `EmpEmployee` (1 employee : N users); the older 1:1 `EmpEmployee.userId` is deprecated, kept in the schema and drained at startup (`migrateEmployeeLinksToUsers`). Enums `EmpDepartment`, `AttendanceStatus`. `Tenant` gains back-relations only. Employee delete is a **hard delete** (schedule rows cascade). All additive `db push` (no data loss).
...
#### v2.93.0 — Partial Day + redesigned tabs

- **Partial Day** (`AttendanceStatus.PARTIAL_DAY`, additive enum value) stores its hours in `emp_schedules.worked_hours` (nullable Float, 0.5–7.5 in 0.5 steps; NULL for every other status and for all rows saved before v2.93.0). Shared helpers `cellWorkedHours(status, ot, workedHours)` and `cellDayFactor(status, workedHours)` are the single source for grid, report, exports and the Warehouse Report factor (Partial = hours ÷ 8; live shift = its hours). Present / Half Day formulas unchanged — verified identical on a copy of all prod rows (4 months, 17 weeks).
- **Bulk fill** (`fillEmptyCells`) only ever writes empty cells (`createMany skipDuplicates` on the tenant/employee/date unique key); `copyPreviousWeek` reuses it; `undoBulkFill` deletes exactly the created cells while they still hold the same status. Routes: `POST /employee-schedule/schedule/fill | copy-previous-week | undo-fill` (write guard).
- **Read-only endpoints:** `GET /employees/attendance-summary?days=30|90|180`, `GET /employees/:id/history?days=` (profile panel), `GET /report/analytics?from=&to=&department=` (≤ 186 days + same-length previous range, daily counts, per-department totals, per-employee summaries, status grid for ≤ 62 days). Exports accept `?from=&to=` (`getReportForRange` shares the week/month aggregation via `buildReport`).
- **Frontend:** `pages/employeeSchedule/{ScheduleTab, CellEditor, scheduleUi, EmployeesTab, EmployeeProfile, ReportTab}.tsx` + `styles/employeeSchedule.css` (`.es-*` grid, `.ee-*` employees/profile, `.er-*` report); Report reuses the Marketing chart kit (`.mkt-root`).
...
| GET | `/users` | ADMIN | List users (incl. `employee.empNo` of the linked Employee Schedule record) |
| POST | `/users` | ADMIN | Create user |
| PATCH | `/users/:id` | ADMIN | Update/deactivate user. `employeeNo` (v2.86.0, any role since v2.87.0) links the login to an Employee Schedule ID; `null` unlinks; unknown / left-company ID → 400. Several logins may share one ID (v2.88.0). |
...
| **Live Performance — live floor** (v2.85.0: attention cards, Pickers/Packers scoreboards with end-of-shift projection, hourly Pickers-vs-Packers chart, live leaderboard with state + progress-to-target + projection, worker × hour heatmap) | Warehouse Report → Live Performance tab | Today (live) or any historical date up to 90 days back, Manila TZ — live: socket-pushed (`order:stats_changed`) + 30s polling; historical: no refetch, 5-min stale window |
...
- v2.84.0: per-worker hourly charts hide inactive users; the per-worker tables render **only for today** (the historical "Breakdown" tables were removed — the Performance tab replaces them)

#### Live floor board — data model (v2.85.0)
- Endpoint: `GET /reports/live-board?date=YYYY-MM-DD` (read-only, same RBAC as `/performance`; service `getLiveBoard` in `performanceService.ts`). No date / today = live; a past day (≤ 90 days) = replay with final numbers; future / older / malformed → `400`. Both roles in one response. The v2.x `GET /reports/live-performance` endpoint is still served but no longer used by the UI
- Workers = active PICKER / PACKER users minus those whose linked Employee Schedule record is inactive (same as the target report)
- Per worker: hourly completions (Postgres, Manila +8h), last-hour count, first / last completion, open work = same definition as the Picker/Packer Admin workload cards (`…_ASSIGNED` = queued, `PICKING`/`PACKING` = in hand, order not archived)
- Target: Present = 1 × daily target, Half Day = 0.5, Day Off / leave / Absent = 0 (1 if they produced anyway), no schedule entry = 1 when there is output or work, else 0. Shift length: Present 8h + OT, Half Day 4h, otherwise 8h (`LIVE_DEFAULT_SHIFT_HOURS`)
...
- **Output** = completed `PickerAssignment` / `PackerAssignment` rows per Manila day, aggregated in Postgres (`(completed_at + interval '8 hours')::date`, naive-UTC bounds cast `::timestamp`); parity with the original Excel report verified to the decimal
- **Workers** = active `PICKER`/`PACKER` users, minus anyone whose linked Employee Schedule record is inactive
- **Attendance link:** `User.employeeId` (v2.88.0, nullable FK → `emp_employees`, one employee can own several logins; replaces the deprecated 1:1 `EmpEmployee.userId`, drained at startup) ties a login to its Employee Schedule record; set in Employee Schedule → Employees → Edit → *Linked system logins* (`userIds[]`, omitted = unchanged, a login linked to another employee → `409`) or Settings → Edit user → *Employee ID* (`PATCH /users/:id { employeeNo }`, null unlinks). The performance report reads picker/packer links.
- **Day factor:** Present = 1 (full target) · Half Day = 0.5 · Day Off / leave / Absent = 0 (excluded) unless output > 0 (then 1, flagged `WORKED_ON_LEAVE`) · no schedule entry = 1 if output > 0 (flagged `NO_SCHEDULE` when linked) else 0
- **Per worker:** total output, active days (Σ factor), target output, avg / active day, achievement % (total ÷ target output), days met target, hit rate, best day, status (Achieved ≥ 100% · Near 90–99.9% · Below < 90%), rank by avg / active day
- **Today** (if in range) is shown but excluded from every total/rate (`inProgress`); "Last N days" presets end yesterday
- Max window 92 days (`PERF_MAX_RANGE_DAYS`); invalid/future/too-long ranges → `400`
- Excel export is generated client-side by `frontend/src/lib/xlsxWriter.ts` (no spreadsheet dependency) — Summary + Daily Performance sheets mirroring the original workbook
- Files: `frontend/src/pages/reports/TargetPerformanceTab.tsx`, `EmployeePerformanceTab.tsx`, `perf/perfUi.tsx`, `perf/exportTargetXlsx.ts`, `frontend/src/styles/performance.css`, `shared/src/performance.ts`
