...
*UI.* A shared **Attachments** section (`components/accounting/InvoiceAttachments.tsx`, `resource: 'sales' | 'expenses'`) sits under Items/Note in `InvoiceForm` and `PurchaseForm`. **Hovering** a saved or queued file shows an enlarged preview — the actual image for JPG/PNG/WEBP, a PDF icon + filename card for PDFs (no client-side PDF rendering). In edit mode a file uploads on selection. On a **new** record there is no id yet, so files queue client-side and flush after the save returns one; if a flush partially fails the form stays put, keeps the failed files, and records the new id so pressing Save again is a `PUT` retry rather than a duplicate `POST`. Previews and downloads fetch **blobs through the api client** — a bare `<img src>` or `window.open` sends no credentials and gets nginx's SPA fallback, i.e. the login screen (SOLUTIONS `[2026-05-02]` / `[2026-06-03]`).
