...
**Expense attachments (v2.77.0):** an expense can carry the supplier invoice that backs it — photo or PDF. New table `AccExpenseAttachment` (`expenseId` cascade, `fileName`, `mime`, `originalName`, `sizeBytes`, `uploadedAt`); the bytes live on the `backend_uploads` volume under `accounting/`, and the row stores **only the file name** (never an absolute path, so a change of `UPLOADS_ROOT` — dev Windows vs prod `/app/uploads` — cannot orphan it; this is the latent bug `IncidentDocument.filePath` still carries). Endpoints `GET|POST /accounting/expenses/:id/attachments` and `GET|DELETE /accounting/expenses/:id/attachments/:attId`, all under the existing `ADMIN + ACCOUNTANT` guard. Uploads are capped at **10 MB**, MIME-whitelisted to PDF/PNG/JPG/WEBP (**SVG is deliberately excluded** — it is script-bearing and these files are served back `inline`), served with `X-Content-Type-Options: nosniff`, and a re-upload of the same name+type returns **409**. Oversize returns a friendly **413** (`@fastify/multipart` throws from `toBuffer()`; the `truncated` check is a fallback — see SOLUTIONS `[2026-06-21]`).

*Storage discipline.* Photos are **downscaled in the browser before upload** (`lib/imageDownscale.ts`: long edge → 1600 px, JPEG q0.82, skipped under 600 KB or if the result is larger). A ~3 MB phone photo lands around 400 KB. This matters because these are statutory records (BIR, ~10 years) and are therefore **exempt from every retention/purge job** — the volume only ever grows, so the ~8× saving is what keeps it bounded. PDFs pass through untouched.

*File lifecycle.* Soft-deleting an expense **keeps** its files (the Recycle Bin must be able to restore it). Only a permanent purge unlinks them, and `purgeExpense` reads the file names **before** the delete — the cascade would otherwise drop the attachment rows and with them the only record of which files on disk belonged to that expense, leaving orphans nobody can find. Deleting a single attachment unlinks immediately.

*UI.* An **Attachments** section sits under Items in `PurchaseForm`. In edit mode a file uploads on selection. On a **new** expense there is no id yet, so files queue client-side and flush after the save returns one; if a flush partially fails the form stays put, keeps the failed files, and records the new id so pressing Save again is a `PUT` retry rather than a duplicate `POST`. Previews and downloads fetch **blobs through the api client** — a bare `<img src>` or `window.open` sends no credentials and gets nginx's SPA fallback, i.e. the login screen (SOLUTIONS `[2026-05-02]` / `[2026-06-03]`).
