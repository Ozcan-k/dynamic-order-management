...
- **Attendance link:** `EmpEmployee.userId` (nullable, unique) ties an employee to their system login (any role since v2.87.0; the performance report uses picker/packer links); set in Employee Schedule → Employees → Edit → *Linked system user* or (v2.86.0) Settings → Edit user → *Employee ID* (`PATCH /users/:id { employeeNo }`, null unlinks) (`GET /employee-schedule/linkable-users`). A PUT without `userId` leaves the link unchanged; linking a user twice → `409`
