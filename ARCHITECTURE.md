...
> **Note (v2.46.0):** the **Inbound Admin** role is now labelled **"Inbound/Outbound Admin"** in Settings (enum value unchanged: `INBOUND_ADMIN`). In addition to the surfaces above it can reach the **Return & Cancel** panel + scan (Section 7.11). The matrix above predates `WAREHOUSE_ADMIN`, `SALES_AGENT` and `STOCK_KEEPER`; see Sections 6 (Sales Agent / Stock Keeper) and the per-route guards in `App.tsx` for the authoritative current access map.
...
`Sidebar.tsx` gained an `IncidentIcon` and a new nav entry "Incident Report" placed right after "Marketing Report" — `roles: [UserRole.ADMIN]`. `App.tsx` route `/incident-report` is wrapped in `ProtectedRoute allowedRoles={[ADMIN]}`. Non-admin sessions get redirected as usual.
...
### 7.11 Return & Cancel Parcel Module ✅ Built (v2.46.0)

**Visible to:** Admin, Warehouse Admin, **Inbound/Outbound Admin** (`INBOUND_ADMIN`)
**Sidebar:** `Return & Cancel` parent under **Outbound**, with two children — **Scan** (`/returns/scan`) and **Report** (`/returns`).

Tracks parcels that come back as **returns** or get **cancelled**, outside the normal inbound → outbound lifecycle.

**Scan screen (`/returns/scan`)** — handheld-friendly, the operator's flow top-to-bottom:
1. **Waybill** — auto-focused text input (handheld scanner types straight in).
2. **Type** — dropdown: `Return` / `Cancel`.
3. **Store** — dropdown of the 17 `SALES_STORES`.
4. **Platform** — dropdown `Shopee` / `Lazada` / `TikTok` (`RETURN_CANCEL_PLATFORMS`); **auto-detected** from the waybill prefix via the existing `detectPlatform()` until the user overrides it.
5. **Courier** — dropdown of the `Carrier` enum (`CARRIER_LABELS`).

On Save the row persists and the form resets the waybill (keeping Type/Store/Courier sticky for fast repeated scanning); a "Recent Scans" list shows the session's saves for visual confirmation. The form is an HTML `<form>`, so a scanner's trailing Enter submits.

**Report screen (`/returns`)** — summary cards (**Total / Returns / Cancels**), a **Search Waybill** + **Type** filter, and a **date-range strip** with `1 Day` / `7 Days` / `1 Month` / `Custom` presets (Incident-style date picker, Manila `YYYY-MM-DD`, interpreted as UTC+8). Paginated table (25/page) with columns Waybill · Type · Store · Platform · Courier · Date/Time · Delete.

**Data model:** new table `return_cancel_parcels` (`id`, `tenant_id`, `tracking_number`, `type ReturnCancelType`, `store_name`, `platform Platform`, `carrier Carrier`, `created_by`, `created_at`), indexed on `(tenant_id, created_at desc)` and `(tenant_id, type)`. New enums `ReturnCancelType {RETURN, CANCEL}` and `Carrier {SPX, JT_EXPRESS, FLASH, LEX, LBC, NINJA_VAN, OTHER}` added to Prisma (Carrier had previously existed only as a shared TS enum; `orders.carrier_name` remains a free String, untouched).

**Retention:** records are **hard-deleted after 180 days (6 months)** by the nightly job — `returnCancelService.hardDeleteExpiredReturnCancel(tenantId)` is called per-tenant inside `nightlyReport.ts` alongside the existing order purge. (Independent of the Stock retention exemption.)

**Backend:** `routes/returns.ts` (GET list+stats / POST create / DELETE :id, all gated `requireRole(ADMIN, WAREHOUSE_ADMIN, INBOUND_ADMIN)`) registered at prefix `/returns` in `index.ts`; `services/returnCancelService.ts`. **Frontend:** `api/returns.ts` (React Query hooks), `pages/ReturnCancelScan.tsx`, `pages/ReturnCancel.tsx`. **Proxy:** `returns` prefix added to `vite.config.ts proxyRoutes` + `nginx.conf` location regex (otherwise the SPA fallback would 200-HTML the backend calls).

**Role rename:** `INBOUND_ADMIN`'s Settings display label changed `Inbound Admin → Inbound/Outbound Admin` (plural `Inbound/Outbound Admins`). The **enum value stays `INBOUND_ADMIN`** — no DB migration; existing users keep working. This role gains access to the Return & Cancel panel + scan in addition to its prior surfaces.
...
/archive              → ADMIN only
/users                → ADMIN only
/incident-report      → ADMIN only  (v2.43.0)
```
