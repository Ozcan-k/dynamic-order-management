...
> **Version:** 2.78.0  
> **Date:** 2026-07-09  
> **Status:** **v2.85.0 (test)** — **Live Performance rebuilt as a live floor board** (`GET /reports/live-board`): per-worker live state (Working / Idle 20+ min / Not started / Shift done / Off), pace, end-of-shift projection vs target, attention cards, role scoreboards, leaderboard and a worker × hour heatmap. No schema change. See Section 13.
>
...
- **Attendance link:** `EmpEmployee.userId` (nullable, unique) ties an employee to the picker/packer login; set in Employee Schedule → Employees → Edit → *Linked system user* (`GET /employee-schedule/linkable-users`). A PUT without `userId` leaves the link unchanged; linking a user twice → `409`
