...
>
> **v2.79.0 — Stock Transfer kaldırıldı.** Scan operasyonları artık yalnız **Stock In / Stock Out**. `StockScan.tsx`'ten tüm transfer UI'ı, `api/stock.ts`'ten `ScanOperation` `TRANSFER`/`ScanPayload.toWarehouseId`, backend `ScanSchema` enum'undan `TRANSFER` (deneme → 400) ve `scanItem` TRANSFER dalı çıkarıldı. `MovementType` enum'undaki `TRANSFER` **schema'da kalır** (geçmiş hareketler + `transfers30d` bozulmasın). Ayrıca **bir defalık prod düzeltmesi**: Quezon→Annonas transfer edilip hâlâ Annonas'ta IN_STOCK olan **70 ürün / 335 kutu / 11.749,30** `ADJUSTMENT_OUT` ile `OUT_OF_STOCK` yapıldı (in-stock toplamından düştü; doğrudan Annonas stock-in'e dokunulmadı; şema değişmedi).

---

## v2.33.1 – v2.36.1 değişiklik özeti (2026-05-14 – 2026-05-21)
...
| **v2.36.2** | **Scan UX feedback bundle — daha hızlı banner + daha güçlü ses/titreşim** — Saha: scan onayından sonra yeşil "↓ Stocked" / kırmızı error banner 7 saniye ekranda kalıyordu, operatör sonraki kutuyu tararken kafa karışıklığı yaratıyordu. (a) `RESULT_TOAST_MS` 7000 → 2000 (banner 5 sn daha kısa). (b) `playBeep(true)` artık 3-nota yükselen chirp (880→1175→1480 Hz, gain 0.35→0.6, 200ms→450ms) — gürültülü depo ortamında net duyuluyor. (c) `playBeep(false)` 2-nota inen square buzz (220→180 Hz, gain 0.2→0.45, 300ms→500ms) — success'ten net ayırt edilebiliyor. (d) Confirm success vibrate `[200,60,80,60,80]` → `[250,100,200,100,200]` (3 güçlü pulse, ~870ms). (e) Confirm error vibrate `[100,60,100,60,100]` → `[180,100,180,100,180,100,280]` (4 pulse, ~1180ms — eldivenle bile hissedilir). Bulk-commit sonundaki feedback de aynı pattern'a eşitlendi. Bulk mode log persistent kalmaya devam ediyor (batch review için). |
| **v2.36.1** | **Single Scan double-fire fix** — Saha şikayeti: yeni basılan etiketler bile Stock In yaparken "Already stocked at W2 - Quezon Warehouse" hatası dönüyordu. Kök sebep `StockScan.tsx` single mode'da: scan başarılı olduktan sonra `setPendingScan(null)` modal'ı kapatıyordu ama kamera + zxing `decodeFromStream` kesintisiz aktif kaldığı için aynı QR hâlâ viewfinder'da → bir sonraki frame tekrar `setPendingScan({id})` fire ediyordu (single mode'da `lockedRef` koruması yoktu, sadece bulk mode'da var). Operatör flash olan yeşil success'i fark etmeden modal'ı tekrar görüp Confirm'e basınca server bu kez IN_STOCK durumdaki row için v2.34.1 strict re-IN block'una takılıyordu. Fix: `lastResolvedIdRef` adlı tek-slot guard eklendi — confirmScan onSuccess/onError'da scan edilmiş ID'yi kaydediyor; decode callback aynı ID'yi tekrar görürse silent skip ediyor. Farklı bir QR algılanırsa veya operatör Cancel'a basarsa ya da kamerayı kapatırsa guard reset oluyor. Backend (scanItem state machine) hiç değişmedi — yalnız client-side debounce. |
