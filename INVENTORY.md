...
## v2.33.1 – v2.34.5 değişiklik özeti (2026-05-14 – 2026-05-15)
...
| **v2.35.4** | **Label PDF — product name full-render fix** — Eski `fitText` 18 mm text alanına sığmayan ürün adlarını "Dried Di…" gibi ellipsis ile kesiyordu (saha fotoğrafı ile teyit edildi). `stockService.ts`'ye yeni `fitProductName(doc, text, maxWidth)` helper'ı eklendi: önce 10pt tek satır dener; sığmazsa 10pt'de greedy word-wrap ile 2 satıra böler; 2 satır da yetmezse font'u 10→9→8→7pt'ye indirip her boyutta 1+2 satır kombinasyonlarını dener. Worst-case (7pt'de bile tek kelime aşıyor) ellipsis fallback kalır. Y konumu lineY(5) sabit, 2. satır `size × 1.2` line-height ile aşağı yazılır — 10pt iki satır ~y=9.2 mm'de bitiyor, qty satırı (y=15 mm) ile 5.8 mm açıklık. QR boyutu (36 mm), payload (raw UUID), QR settings (EC=M margin 4) dokunulmadı; sadece render-side fix. Schema değişmedi. |
