...
export default function PackerAdminScan() {
  ...
  const singleMutation = useMutation({
    mutationFn: (tn: string) =>
      api.post<{ order: { trackingNumber: string } }>('/packer-admin/scan', { trackingNumber: tn }),
    onSuccess: (res) => {
  ...
  const isPending = singleMutation.isPending || bulkValidateMutation.isPending
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg, #0f172a 0%, #052e1a 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ width: '100%', maxWidth: '480px', padding: '20px 20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: 42, height: 42, borderRadius: '12px', background: 'linear-gradient(135deg, #16a34a, #15803d)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 14px rgba(22,163,74,0.45)', flexShrink: 0 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M16 16h6"/><path d="M21 12.5V19a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h11"/><path d="M11 12l3 3 7-7"/>
            </svg>
          </div>
          <div>
            <div style={{ color: '#f1f5f9', fontWeight: 800, fontSize: '17px', lineHeight: 1.2, letterSpacing: '-0.2px' }}>Packer Admin Scan</div>
            <div style={{ color: '#64748b', fontSize: '13px', marginTop: '1px' }}>{user?.username}</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: '#86efac', background: 'rgba(22,163,74,0.2)', padding: '6px 14px', borderRadius: '9999px', border: '1px solid rgba(22,163,74,0.35)', letterSpacing: '0.06em' }}>
            HANDHELD
          </div>
      ...
      <div style={{ width: '100%', maxWidth: '480px', padding: '20px 16px 32px', boxSizing: 'border-box', flex: 1 }}>
        ...
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.07)', borderRadius: '16px', padding: '5px', marginBottom: '24px', border: '1px solid rgba(255,255,255,0.08)' }}>
          {(['single', 'bulk'] as ScanMode[]).map(m => (
            <button key={m} onClick={() => { setMode(m); setFeedback(null) }} style={{
              ...
              boxShadow: mode === m ? '0 2px 10px rgba(22,163,74,0.55)' : 'none',
              transition: 'all 0.2s',
            }}>
        ...
        {/* Info card */}
        <div style={{ background: 'rgba(22,163,74,0.1)', border: '1px solid rgba(22,163,74,0.22)', borderRadius: '14px', padding: '14px 18px', marginBottom: '24px', display: 'flex', gap: '12px', alignItems: 'center' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#86efac" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>
          </svg>
          <span style={{ fontSize: '14px', color: '#bbf7d0', lineHeight: 1.6, fontWeight: 500 }}>
            {mode === 'single'
              ? 'Scan one waybill — it appears in the desktop staging area to assign to a packer.'
        ...
        <button
          ...
          style={{
            ...
            background: isPending || cameraOpen ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #16a34a, #15803d)',
            cursor: isPending || cameraOpen ? 'not-allowed' : 'pointer',
            boxShadow: isPending || cameraOpen ? 'none' : '0 10px 36px rgba(22,163,74,0.5)',
        ...
        {mode === 'bulk' && bulkItems.length > 0 && (
          <div>
            ...
            <button onClick={() => setBulkItems([])} style={{ width: '100%', padding: '18px 0', borderRadius: '16px', border: '1px solid rgba(22,163,74,0.35)', background: 'rgba(22,163,74,0.12)', color: '#86efac', fontSize: '17px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', transition: 'all 0.2s' }}>
